#include "Gyroscope.h"
#include "stdafx.h"
#include "arduino.h"

#define GYRO 0x68         // gyro I2C address
#define REG_GYRO_X 0x1D   // IMU-3000 Register address for GYRO_XOUT_H
#define ACCEL 0x53        // Accel I2c Address
#define ADXL345_POWER_CTL 0x2D

byte buffer[12];   // Array to store ADC values 
int16_t gyro_x;
int16_t gyro_y;
int16_t gyro_z;
int16_t accel_x;
int16_t accel_y;
int16_t accel_z;
int i;
#include <Wire.h>

Gyroscope::Gyroscope()
{
	lastMeasureTime = 0;
	lastAngleY = 0;
}


Gyroscope::~Gyroscope()
{
}

void Gyroscope::init()
{
	Wire.begin();
	// Set Gyro settings
	// Sample Rate 1kHz, Filter Bandwidth 42Hz, Gyro Range 500 d/s 
	writeTo(GYRO, 0x16, 0x0B);
	//set accel register data address
	writeTo(GYRO, 0x18, 0x32);
	// set accel i2c slave address
	writeTo(GYRO, 0x14, ACCEL);
	// Set passthrough mode to Accel so we can turn it on
	writeTo(GYRO, 0x3D, 0x08);
	// set accel power control to 'measure'
	writeTo(ACCEL, ADXL345_POWER_CTL, 8);
	//cancel pass through to accel, gyro will now read accel for us   
	writeTo(GYRO, 0x3D, 0x28);
}

void Gyroscope::writeTo(int device, byte address, byte val) {
	Wire.beginTransmission(device); // start transmission to device 
	Wire.write(address);             // send register address
	Wire.write(val);                 // send value to write
	Wire.endTransmission();         // end transmission
}

void Gyroscope::rawRead(int16_t &accX, int16_t &accY, int16_t &accZ, int16_t &gyroX, int16_t &gyroY, int16_t &gyroZ)
{
	// Read the Gyro X, Y and Z and Accel X, Y and Z all through the gyro
	// First set the register start address for X on Gyro  
	Wire.beginTransmission(GYRO);
	Wire.write(REG_GYRO_X); //Register Address GYRO_XOUT_H
	Wire.endTransmission();

	byte buffer[12]; // Array to store IMU values 

	// New read the 12 data bytes
	Wire.requestFrom(GYRO, 12); // Read 12 bytes
	i = 0;
	while (Wire.available())
	{
		buffer[i] = Wire.read();
		i++;
	}

	//Combine bytes into integers
	// Gyro format is MSB first
	gyroX = (int16_t)(buffer[0]) << 8 | buffer[1];
	gyroY = (int16_t)(buffer[2]) << 8 | buffer[3];
	gyroZ = (int16_t)(buffer[4]) << 8 | buffer[5];

	accY = (int16_t)(buffer[7]) << 8 | buffer[6];
	accX = (int16_t)(buffer[9]) << 8 | buffer[8];
	accZ = (int16_t)(buffer[11]) << 8 | buffer[10];
}

void Gyroscope::getAngleY(float &phi, float &deltaPhi)
{
	int16_t x, y, z, dx, dy, dz;

	int mgY = 0;
	int maY = 0;
	int maZ = 0;

	int measuresCount = 2;

	for (int i = 0; i < measuresCount; i++)
	{
		rawRead(x, y, z, dx, dy, dz);
		mgY += dy;
		maY += y;
		maZ += z;
	}
	mgY /= measuresCount;
	maY /= measuresCount;
	maZ /= measuresCount;

	// chip scale: 15bit = 500 deg/sec
	deltaPhi = -mgY * 500.0 / 32768;

	phi = -atan2f(maY, maZ) * 180 / PI;
}

void Gyroscope::calibrate()
{
	int calibrationEnd = millis() + 2 * 1000; // 2 seconds

	int iterationCount = 0;
	float meanPhi = 0;
	float meanDeltaPhi = 0;

	while (millis() < calibrationEnd)
	{
		float phi, deltaPhi;
		getAngleY(phi, deltaPhi);
		meanPhi += phi;
		meanDeltaPhi += deltaPhi;
		iterationCount++;
	}

	phi0 = meanPhi / iterationCount;
	deltaPhi0 = meanDeltaPhi / iterationCount;

	Log(L"Calibration done. Phi0=%f\t Delta0=%f\n", phi0, deltaPhi0);
	lastMeasureTime = 0;
	lastAngleY = 0;
}

void Gyroscope::getAngleFiltered(float &phi, float &deltaPhi, unsigned long measureTime)
{
	float p, d;
	getAngleY(p, d);
	p -= phi0;
	d -= deltaPhi0;

	float result = 0;

	result = lastAngleY * 0.95;

	if (lastMeasureTime > 0)
	{
		result += d * (measureTime - lastMeasureTime) / 1000.0 * 0.95;
	}

	if (fabs(p) < 30)
	{
		result += 0.05 * p;
	}

	lastAngleY = result;
	lastMeasureTime = measureTime;
	phi = result;
	deltaPhi = d;
}